...
class DeepEarthMLPDataset(Dataset):
    ...
    def __init__(self, observation_ids, cache, mode='both', device='cpu', species_mapping=None, 
                 min_samples_per_species=2, use_bfloat16=False):
    ...
    def _load_dataset(self):
        ...
        # Build species mapping from ALL loaded data if not provided
        if self.species_mapping is None:
            ...
            valid_species = [s for s, count in species_counts.items() 
                           if count >= self.min_samples_per_species]
            
            if not valid_species:
                raise ValueError(f"No species with >= {self.min_samples_per_species} samples found!")
            
            self.species_to_idx = {species: idx for idx, species in enumerate(sorted(valid_species))}
        else:
        ...
        if not filtered_indices:
            raise ValueError("No observations found for mapped species!")
        ...
        self.observation_ids = [all_obs_ids[i] for i in filtered_indices if i < len(all_obs_ids)]
        
        # Store embeddings on CPU by default, move to device in __getitem__
        if self.mode in ['language', 'both'] and all_language_embs:
            all_language = torch.cat(all_language_embs, dim=0)
            self.language_embeddings = all_language[filtered_indices]
            if self.use_bfloat16:
                self.language_embeddings = self.language_embeddings.to(dtype=torch.bfloat16)
            
        if self.mode in ['vision', 'both'] and all_vision_embs:
            all_vision = torch.cat(all_vision_embs, dim=0)
            self.vision_embeddings = all_vision[filtered_indices]
            if self.use_bfloat16:
                self.vision_embeddings = self.vision_embeddings.to(
        ...
        unique_species, counts = torch.unique(self.species_labels, return_counts=True)
        logger.info(f"Species distribution: min={counts.min().item()}, max={counts.max().item()}, "
    ...
    def __getitem__(self, idx):
        ...
        sample = {
            'species_idx': self.species_labels[idx].to(self.device),
            'species': self.idx_to_species[self.species_labels[idx].item()],
            'gbif_id': self.observation_ids[idx] if idx < len(self.observation_ids) else 'unknown'
        }
        
        if self.mode in ['language', 'both']:
            sample['language_embedding'] = self.language_embeddings[idx].to(self.device, non_blocking=True)
            
        if self.mode in ['vision', 'both']:
            sample['vision_embedding'] = self.vision_embeddings[idx].to(self.device, non_blocking=True)
...
def create_umap_visualization(embeddings, labels, species_names, save_path, title="UMAP", max_points=2000):
    """Create UMAP visualization with subsampling for speed"""
    # Subsample if needed
...
def main():
    ...
    parser.add_argument('--min-samples-per-species', type=int, default=3, help='Min samples per species')
    parser.add_argument('--seed', type=int, default=42, help='Random seed')
    parser.add_argument('--use-bfloat16', action='store_true', help='Use bfloat16 for embeddings')
    ...
    # First load both datasets to build consistent species mapping
    print("\nBuilding species mapping...")
    temp_train = DeepEarthMLPDataset(
        train_ids, cache, args.mode, 'cpu', 
        species_mapping=None,
        min_samples_per_species=args.min_samples_per_species,
        use_bfloat16=args.use_bfloat16
    )
    
    # Get common species between train and potential test
    train_species = set(temp_train.species_to_idx.keys())
    print(f"Train species: {len(train_species)}")
    
    # Create final datasets with consistent mapping
    species_mapping = temp_train.species_to_idx
    species_names = temp_train.idx_to_species
    
    train_dataset = temp_train  # Reuse already loaded
    test_dataset = DeepEarthMLPDataset(
        test_ids, cache, args.mode, args.device, 
        species_mapping=species_mapping,
        min_samples_per_species=1,  # Allow any species in test that's in train
        use_bfloat16=args.use_bfloat16
    )
    
    # Verify species overlap
    train_species_set = set(train_dataset.species_labels.cpu().numpy())
    test_species_set = set(test_dataset.species_labels.cpu().numpy())
    common_species = train_species_set & test_species_set
    ...
    # CSV logging - check if file exists to avoid duplicate headers
    ...
    dummy_vision = torch.zeros(args.batch_size, 8, 24, 24, 1408, device=args.device, dtype=torch.bfloat16 if args.use_bfloat16 else torch.float32)
    dummy_language = torch.zeros(args.batch_size, 7168, device=args.device, dtype=torch.bfloat16 if args.use_bfloat16 else torch.float32)
    ...
    for epoch in range(args.epochs):
        ...
        for batch in progress:
            ...
            # Generate language mask with proper dtype
            if args.mode in ['language', 'both'] and args.mask_ratio > 0:
                language_mask = (torch.rand_like(batch['language_embedding']) < args.mask_ratio).bool()
            else:
                language_mask = None
            
            # Prepare inputs with dummy tensors if needed
            if args.mode in ['vision', 'both']:
                vision_input = batch['vision_embedding']
            else:
                vision_input = dummy_vision[:batch_size]
                
            if args.mode in ['language', 'both']:
                language_input = batch['language_embedding']
            else:
                language_input = dummy_language[:batch_size]
