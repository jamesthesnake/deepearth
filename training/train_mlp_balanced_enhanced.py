...
class DeepEarthDataset(Dataset):
    ...
    def __init__(self, observation_ids, cache, species_mapping=None, mode='both', 
        ...
        if hard_negatives_file and os.path.exists(hard_negatives_file):
            ...
            self.hard_negatives = torch.load(hard_negatives_file,weights_only=False)
...
class EnhancedMLPUNet(nn.Module):
    ...
    def __init__(self, vision_dim=1408, language_dim=7168, hidden_dim=2048, 
        ...
        self.vision_proj = nn.Sequential(
            ...
            # Removed LayerNorm with elementwise_affine=False - can cause collapse
        )
    ...
    def forward(self, vision_emb, language_emb, return_features=False):
        ...
        # L2 normalize AFTER projection (not using LayerNorm)
        vision_proj = F.normalize(vision_proj, dim=1, p=2)
        language_proj = F.normalize(language_proj, dim=1, p=2)
...
def compute_losses_with_queue(outputs, labels, moco_queue=None, temperature=0.1, 
                             alpha_cls=1.0, alpha_cont=1.0, alpha_var=0.01):
    """Compute losses with MoCo-style queue and variance regularization"""
    losses = {}
    ...
    vision_var = torch.var(vision_proj, dim=0).mean()
    language_var = torch.var(language_proj, dim=0).mean()
    var_loss = -torch.log(vision_var + 1e-6) - torch.log(language_var + 1e-6)
    losses['var_reg'] = var_loss
    ...
    if moco_queue is not None:
        ...
        mask_value = torch.finfo(vision_proj.dtype).min if vision_proj.dtype == torch.float16 else -1e9
        l_neg_batch = l_neg_batch.masked_fill(mask, mask_value)
        ...
        v_neg_batch = v_neg_batch.masked_fill(mask, mask_value)
    ...
    # Total loss
    losses['total'] = alpha_cont * losses['contrastive'] + alpha_cls * losses['classification'] + alpha_var * losses['var_reg']
...
def mine_hard_negatives(model, dataset, device, k=10):
    ...
    running_idx = 0
    with torch.no_grad():
        for batch in tqdm(temp_loader, desc="Mining hard negatives"):
            ...
            # Use running index counter for accurate indices
            batch_size = len(batch['species_idx'])
            all_indices.extend(range(running_idx, running_idx + batch_size))
