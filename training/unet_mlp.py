...
# Keep your existing EfficientMultimodalDataset class exactly as is
...
class MultimodalAutoencoderWithHierarchicalUNet(nn.Module):
    ...
    - Hierarchical U-Net for spatial multimodal fusion
    - Integrated MLP decoders within the architecture
    - Support for simultaneous vision and language masking
    """
    
    def __init__(self, num_classes, vision_dim=1408, language_dim=7168, 
                 universal_dim=2048, hidden_dim=512, dropout_rate=0.2,
                 spatial_size=32, unet_features=[32, 64, 128, 256],
        ...
        self.vision_mask_token = nn.Parameter(torch.randn(1, vision_dim))
        self.language_mask_token = nn.Parameter(torch.randn(1, language_dim))
        
        # Initial modality encoders (like your original)
        ...
        # Project to spatial format for U-Net processing
        self.vision_to_spatial = nn.Sequential(
            nn.Linear(universal_dim, self.spatial_dim),
            Rearrange('b (h w) -> b 1 h w', h=spatial_size, w=spatial_size)
        )
        
        self.language_to_spatial = nn.Sequential(
            nn.Linear(universal_dim, self.spatial_dim),
            Rearrange('b (h w) -> b 1 h w', h=spatial_size, w=spatial_size)
        )
        ...
        self.mask_embedding = nn.Sequential(
            nn.Linear(2, 64),  # [is_vision_masked, is_language_masked]
            nn.ReLU(),
            nn.Linear(64, self.spatial_dim),
            Rearrange('b (h w) -> b 1 h w', h=spatial_size, w=spatial_size)
        )
        
        # Build hierarchical U-Net
        self.in_channels = 3  # vision + language + mask_indicator
        self.features = unet_features
        ...
        for feature in self.features:
            self.encoders.append(self._make_encoder_block(in_channels, feature, dropout_rate))
            self.pools.append(nn.MaxPool2d(2))
            in_channels = feature
        
        # Bottleneck with cross-modal attention
        self.bottleneck = self._make_bottleneck(self.features[-1], self.features[-1] * 2, dropout_rate)
        
        if use_spatial_attention:
            self.cross_modal_attention = CrossModalSpatialAttention(self.features[-1] * 2)
        else:
        ...
        for i in range(len(self.features) - 1, 0, -1):
            self.upconvs.append(nn.ConvTranspose2d(self.features[i] * 2, self.features[i], 2, stride=2))
            self.decoders.append(self._make_decoder_block(self.features[i] * 2, self.features[i-1], dropout_rate))
        
        # Final decoder
        self.upconvs.append(nn.ConvTranspose2d(self.features[0], self.features[0], 2, stride=2))
        self.decoders.append(self._make_decoder_block(self.features[0] * 2, self.features[0], dropout_rate))
        ...
        # Project hierarchical features to universal embedding
        total_hierarchical_features = 256 * len(self.features) + self.features[0] * 16
        
        self.to_universal = nn.Sequential(
            nn.Linear(total_hierarchical_features, universal_dim * 2),
        ...
        # Classifier from universal embedding (like your original)
        ...
        # Integrated MLP decoders for reconstruction
    ...
    def _make_bottleneck(self, in_channels, out_channels, dropout):
        return nn.Sequential(
            nn.Conv2d(in_channels, out_channels, 3, padding=1),
            nn.GroupNorm(16, out_channels),
            nn.GELU(),
            nn.Dropout2d(dropout * 1.5),
            nn.Conv2d(out_channels, out_channels, 3, padding=1),
            nn.GroupNorm(16, out_channels),
            nn.GELU()
        )
    ...
    def forward(self, vision, language, mask_vision=False, mask_language=False, 
        ...
        Maintains compatibility with your original interface
        """
        ...
        # Apply masking if requested (exactly like your original)
        vision_mask = None
        language_mask = None
        ...
        if mask_vision and self.training:
            if mask_type == 'sample':
                ...
                vision = torch.where(
                    ...
                    self.vision_mask_token.expand(batch_size, -1),
                    vision
                )
            else:
        ...
        if mask_language and self.training:
            if mask_type == 'sample':
                ...
                language = torch.where(
                    language_mask.unsqueeze(1),
                    self.language_mask_token.expand(batch_size, -1),
                    language
                )
            else:
                # Feature-level masking
        ...
        # Initial encoding to universal dimension
        vision_universal = self.vision_encoder(vision)
        language_universal = self.language_encoder(language)
        
        # Create mask indicator
        mask_indicator = torch.zeros(batch_size, 2, device=device)
        if vision_mask is not None and mask_type == 'sample':
            mask_indicator[:, 0] = vision_mask.float()
        if language_mask is not None and mask_type == 'sample':
            mask_indicator[:, 1] = language_mask.float()
        
        # Convert to spatial representations
        vision_spatial = self.vision_to_spatial(vision_universal)
        language_spatial = self.language_to_spatial(language_universal)
        mask_spatial = self.mask_embedding(mask_indicator)
        
        # Concatenate all modalities for U-Net
        x = torch.cat([vision_spatial, language_spatial, mask_spatial], dim=1)
        ...
        # Features from encoder path
        for i, (features, proj) in enumerate(zip(encoder_features, self.scale_projections)):
            hierarchical_features.append(proj(features))
        ...
        for i, (upconv, decoder) in enumerate(zip(self.upconvs, self.decoders)):
            ...
            if x.size()[2:] != skip_features.size()[2:]:
        ...
        # Final feature map
        final_features = F.adaptive_avg_pool2d(x, (4, 4)).flatten(1)
        hierarchical_features.append(final_features)
        
        # Concatenate all hierarchical features
        all_features = torch.cat(hierarchical_features, dim=1)
        
        # Project to universal embedding (bottleneck)
        bottleneck = self.to_universal(all_features)
        
        # Classification
        logits = self.classifier(bottleneck)
        
        # Decode to individual modalities
        vision_recon = self.vision_decoder(bottleneck)
        language_recon = self.language_decoder(bottleneck)
...
def train_epoch_fixed(model, loader, optimizer, device, lambda_rec=0.1, lambda_contrast=0.1,
    ...
    for i, batch in enumerate(loader):
        ...
        if mask_vision and outputs['vision_mask'] is not None:
            ...
            if mask_config.get('mask_type', 'feature') == 'sample':
                # For sample-level masking, compute loss only on masked samples
                vision_rec_loss = F.mse_loss(
                    outputs['vision_recon'][mask], 
                    outputs['original_vision'][mask]
                )
            else:
                # For feature-level masking
        ...
        if mask_language and outputs['language_mask'] is not None:
            ...
            if mask_config.get('mask_type', 'feature') == 'sample':
                # For sample-level masking, compute loss only on masked samples
                language_rec_loss = F.mse_loss(
                    outputs['language_recon'][mask], 
                    outputs['original_language'][mask]
                )
            else:
                # For feature-level masking
...
def main():
    ...
    try:
        ...
        if args.use_unet:
            ...
            from . import MultimodalAutoencoder  # Import your original model
        ...
        if args.use_unet:
            logger.info(f"  U-Net spatial size: {args.spatial_size}x{args.spatial_size}")
